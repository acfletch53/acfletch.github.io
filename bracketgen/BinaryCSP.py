...
def consistent(assignment, csp, var, value):
	...
	for constraint in csp.binaryConstraints:
		if constraint.affects(var) : #ignore all the vars that the constraint doesn't affect
			...
			if nextVar == None:
				...
				for constraint in csp.binaryConstraints:
					if constraint.affects(nextVar):
						nextVarDegree += 1
					elif constraint.affects(var):
						varDegree += 1
				if varDegree > nextVarDegree:
					nextVar = var
	return nextVar
...
def leastConstrainingValuesHeuristic(assignment, csp, var):
	...
	constraintList = {}
	for value in values:
		constraintList[value] = countConstraints(assignment, csp, var, value)
	values = sorted(constraintList, key=lambda key: constraintList[key], reverse=True)
	return values

def countConstraints(assignment, csp, var, value):
	num = 0
	for constraint in csp.binaryConstraints:
		if constraint.affects(var) :
			otherVariable = constraint.otherVariable(var)
			otherValues = assignment.varDomains[otherVariable];
			for other in otherValues:
				if constraint.isSatisfied(value, other):
					num += 1
	return num
...
def forwardChecking(assignment, csp, var, value):
	...
	for constraint in csp.binaryConstraints:
		if constraint.affects(var):
			otherVariable = constraint.otherVariable(var)
			otherValueRemainingCount = len(domains[otherVariable])
			if (otherValueRemainingCount == 0):
				return None
			for otherValue in domains[otherVariable]:
				if not constraint.isSatisfied(value, otherValue):
					inferences.add((otherVariable, value))
					otherValueRemainingCount -= 1
...
def revise(assignment, csp, var1, var2, constraint):
	inferences = set([])
	"""Question 5"""
	remainingValueCount = len(assignment.varDomains[var2])
	for otherValue in assignment.varDomains[var2]:
		noneWorked = True
		for value in assignment.varDomains[var1]:
...
def maintainArcConsistency(assignment, csp, var, value):
	...
	while(len(queue) != 0):
		...
		for constraint in csp.binaryConstraints:
			tempInferences = []
			if constraint.affects(popped[0]) and constraint.affects(popped[1]):
				tempInferences = revise(assignment, csp, popped[1], popped[0], constraint)
				if tempInferences != None:
					for inf in tempInferences:
						newInferences.append(inf)
				else:
					for tempInference in inferences:
						assignment.varDomains[tempInference[0]].add(tempInference[1])
					return None
		if len(newInferences) > 0: #only add if we make new inferences
			for constraint in csp.binaryConstraints:
				if constraint.affects(popped[0]) and not constraint.affects(popped[1]):
					queue.append((constraint.otherVariable(popped[0]), popped[0]))
		for addedInferences in newInferences:
			inferences.add(addedInferences)
	return inferences
			...
			if constraint.affects(popped[0]) and constraint.affects(popped[1]):
				...
				if tempInferences != None:
					...
					for tempInference in inferences:
						assignment.varDomains[tempInference[0]].add(tempInference[1])
					return None
		if len(newInferences) > 0: #only add if making new inferences
			for constraint in csp.binaryConstraints:
				if constraint.affects(popped[0]) and not constraint.affects(popped[1]):
					queue.append((constraint.otherVariable(popped[0]), popped[0]))
		for addedInferences in newInferences:
